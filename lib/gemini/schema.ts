...
export const GEMINI_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    grammarMarkdown: {
      type: 'string',
      description:
        'Toàn bộ phần ngữ pháp của bài, dưới dạng một chuỗi Markdown DUY NHẤT, không bao gồm phần luyện tập/bài tập hỏi-đáp (練習/Luyện tập). Cấu trúc Markdown PHẢI theo đúng 3 cấp, không được lẫn cấp: "# Ngữ pháp N" (heading cấp 1, N là số thứ tự thường, bắt đầu từ 1) cho mỗi điểm ngữ pháp lớn - giữ tiêu đề tiếng Việt của điểm đó ngay sau, ví dụ "# Ngữ pháp 1: Cách đặt câu hỏi bằng tiếng Trung"; heading cấp 1 CHỈ dùng cho "Ngữ pháp N", không dùng cho bất kỳ thứ gì khác. Nếu điểm ngữ pháp có các đề mục con chữ cái (A, B...) bên dưới, mỗi đề mục con là một "## A. <tiêu đề đề mục con>" (heading cấp 2: chữ cái + dấu chấm + tiêu đề, ví dụ "## A. Câu hỏi với A 不 A") lồng ngay dưới heading cấp 1 của điểm ngữ pháp đó - TUYỆT ĐỐI KHÔNG tách đề mục con thành heading cấp 1 riêng. Trong mỗi điểm ngữ pháp (hoặc đề mục con), mỗi đề mục giải thích có nhãn riêng (Chức năng/Cấu trúc/Khẳng định/Phủ định/Câu hỏi/Cách dùng... - nhãn KHÔNG cố định, lấy nguyên văn từ sách) KHÔNG phải heading - PHẢI là một dòng in đậm viết hoa toàn bộ, ví dụ "**CHỨC NĂNG**", để phân biệt rõ với 2 cấp heading thật ("#"/"##") ở trên. Ngay sau nhãn in đậm là đoạn văn giải thích (nếu sách có). Mỗi câu ví dụ minh hoạ là một khối ĐÚNG 3 dòng liên tiếp theo cú pháp danh sách có số Markdown: dòng 1 là câu chữ Hán có số thứ tự Markdown đứng trước (ví dụ "1. 王先生要不要喝咖啡？"), dòng 2 là pinyin viết NGHIÊNG (bọc trong "*...*") thụt lề ngay dưới dòng 1 (ví dụ "   *Wáng Xiānshēng yào bú yào hē kāfēi?*"), dòng 3 là nghĩa tiếng Việt thụt lề ngay dưới dòng 2, chữ thường không in nghiêng - ba dòng này là MỘT mục danh sách liên tục (không phải 3 mục riêng, không viết dính liền một dòng); cách nhau với khối ví dụ kế tiếp bằng đúng MỘT dòng trắng, số thứ tự tiếp tục tăng dần (2., 3.,...) chứ không bắt đầu lại từ 1 trong cùng một đề mục. Nếu một đề mục tự nó được sách đánh số thành nhiều ý (1. 2. 3...), dùng danh sách có số Markdown ("1. ... 2. ...") ngay dưới nhãn in đậm của đề mục đó, mỗi mục danh sách chứa cả đoạn giải thích lẫn các khối ví dụ 3-dòng riêng của ý đó. Câu ví dụ minh hoạ cách dùng SAI (đánh dấu "*" ở đầu câu trong sách) vẫn viết theo đúng khối 3-dòng này, giữ nguyên dấu "*" ở đầu dòng 1 (đầu câu chữ Hán, không phải cú pháp in nghiêng Markdown). Bảng dữ liệu tham khảo (liệt kê từ vựng/công thức/danh mục, không phải câu ví dụ đánh số) viết thành danh sách gạch đầu dòng Markdown ("- ...") ngay trong đoạn giải thích của đề mục chứa nó. Nếu bảng gốc có từ 3 cột trở lên hoặc nhiều hàng lặp lại cùng cấu trúc, CÓ THỂ dùng cú pháp bảng Markdown (| cột 1 | cột 2 |...) thay vì danh sách gạch đầu dòng nếu điều đó thể hiện rõ ràng hơn - với bảng chỉ 2 cột đơn giản (ví dụ cặp chữ Hán-nghĩa), giữ nguyên danh sách gạch đầu dòng như hiện tại.',
    },
  },
