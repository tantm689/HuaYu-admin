...
3. "grammarMarkdown": toàn bộ phần ngữ pháp của bài dưới dạng MỘT chuỗi Markdown DUY NHẤT. TUYỆT ĐỐI KHÔNG trích xuất phần luyện tập/bài tập hỏi-đáp (練習/Luyện tập) của ngữ pháp.
   ...
   QUAN TRỌNG về câu ví dụ (PHẢI đánh số và xuống dòng rõ ràng, TUYỆT ĐỐI KHÔNG viết dính liền chữ Hán+pinyin+nghĩa Việt trên cùng một dòng): mỗi câu ví dụ minh hoạ là một khối gồm ĐÚNG 3 dòng liên tiếp, theo cú pháp danh sách có số Markdown ("1. ", "2. "... - số thứ tự của ví dụ trong đề mục đó, bắt đầu từ 1, theo đúng số ①②③ sách đã đánh nếu có) áp dụng cho dòng ĐẦU TIÊN của khối (dòng chữ Hán), rồi 2 dòng còn lại (pinyin, nghĩa Việt) thụt lề ngay dưới dùng cú pháp tiếp tục của cùng mục danh sách đó (không phải mục danh sách mới, không phải đoạn văn rời). Cụ thể mỗi khối ví dụ viết đúng theo mẫu sau, xuống dòng thật giữa 3 dòng (không nối chung một dòng):
     \`\`\`
     1. 王先生要不要喝咖啡？
        *Wáng Xiānshēng yào bú yào hē kāfēi?*
        Ngài Vương có muốn uống cà phê hay không?
     \`\`\`
     Dòng 1: câu ví dụ nguyên văn bằng chữ Hán, có số thứ tự Markdown đứng trước. Dòng 2: pinyin của câu, viết NGHIÊNG bằng cú pháp Markdown ("*...*"), thụt lề dưới dòng 1. Dòng 3: bản dịch tiếng Việt, thụt lề dưới dòng 2, chữ thường không in nghiêng. Giữa MỖI khối ví dụ 3-dòng và khối ví dụ kế tiếp, để đúng MỘT dòng trắng và số thứ tự tiếp tục tăng dần (2., 3.,...) - KHÔNG bắt đầu lại từ 1 cho mỗi đề mục nếu đề mục đó có nhiều khối ví dụ liên tiếp thuộc cùng một danh sách. Câu ví dụ minh hoạ cách dùng SAI (đánh dấu "*" ở đầu câu trong sách, dùng để chỉ lỗi cần tránh) vẫn viết theo đúng khối 3-dòng này, PHẢI giữ nguyên dấu "*" ở đầu dòng 1 (đầu câu chữ Hán) - đây là dấu "câu sai" của sách, KHÔNG phải cú pháp in nghiêng Markdown (dấu "*" đứng riêng ở đầu dòng chữ Hán, không bọc quanh chữ như dòng 2 pinyin).
