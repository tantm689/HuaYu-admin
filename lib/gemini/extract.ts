...
3. "grammarMarkdown": toàn bộ phần ngữ pháp của bài dưới dạng MỘT chuỗi Markdown DUY NHẤT. TUYỆT ĐỐI KHÔNG trích xuất phần luyện tập/bài tập hỏi-đáp (練習/Luyện tập) của ngữ pháp.
   QUAN TRỌNG về cấu trúc heading (3 cấp, PHẢI dùng đúng cấp Markdown tương ứng, không được lẫn cấp): mỗi điểm ngữ pháp lớn trong sách (đánh số La Mã I, II, III... hoặc đơn giản là điểm ngữ pháp thứ N nếu sách không đánh số La Mã) là một heading cấp 2 - "## Ngữ pháp N: <tiêu đề tiếng Việt>" (N bắt đầu từ 1, theo đúng thứ tự xuất hiện, dùng SỐ THƯỜNG không phải số La Mã trong markdown dù sách ghi La Mã). Nếu điểm ngữ pháp có các đề mục con đánh chữ cái (A, B, C...) bên dưới, MỖI đề mục con là một heading cấp 3 - "### <chữ cái>. <tiêu đề đề mục con>" (ví dụ "### A. Câu hỏi với A 不 A") ngay dưới heading cấp 2 của điểm ngữ pháp cha - TUYỆT ĐỐI KHÔNG tách mỗi đề mục con thành heading cấp 2 riêng, và TUYỆT ĐỐI KHÔNG dùng heading cấp 2 cho bất kỳ thứ gì khác ngoài "Ngữ pháp N". Các nhãn đề mục giải thích bên trong (Chức năng/Cấu trúc/Khẳng định/Phủ định/Câu hỏi/Thông thường/Sử dụng/Cách dùng... - nhãn KHÔNG cố định, lấy nguyên văn từ sách rồi viết HOA TOÀN BỘ) KHÔNG phải heading - viết thành một dòng in đậm riêng (ví dụ "**CHỨC NĂNG**"), để phân biệt rõ với 2 cấp heading thật ở trên, và PHẢI để đúng MỘT dòng trắng ngay sau dòng nhãn in đậm này trước khi viết đoạn giải thích - TUYỆT ĐỐI KHÔNG viết dính đoạn giải thích ngay sau nhãn trên cùng một dòng hay dòng kế tiếp không cách dòng trắng. Ngay sau dòng trắng đó là đoạn văn giải thích của riêng đề mục đó (nếu sách có).
   ...
   QUAN TRỌNG về bảng dữ liệu tham khảo (KHÁC với câu ví dụ đánh số - đây là bảng liệt kê từ vựng/công thức/danh mục, ví dụ bảng từ chỉ vị trí kèm hậu tố 面/邊, hoặc bảng địa danh theo nhóm): TUYỆT ĐỐI KHÔNG BỎ QUA loại bảng này, và TUYỆT ĐỐI KHÔNG viết lại thành danh sách gạch đầu dòng - PHẢI dùng ĐÚNG cú pháp bảng Markdown thật (dòng tiêu đề "| Cột 1 | Cột 2 |", dòng phân cách "| --- | --- |" ngay sau, rồi các dòng dữ liệu "| ... | ... |") để giữ đúng số cột và cấu trúc hàng-cột như bảng gốc trong sách, kể cả khi bảng gốc chỉ có 2 cột đơn giản - không rút gọn bảng thành danh sách dù bất kỳ lý do gì. Giữ đúng tiêu đề cột (ví dụ "Địa điểm", "Vị trí", "Hậu tố") làm dòng tiêu đề bảng, và đúng cặp chữ Hán - nghĩa/phiên âm đi kèm trong từng ô của bảng gốc. Nếu bảng gốc có nhiều dòng con lồng trong 1 hàng lớn (ví dụ nhiều mục vị trí đều dùng chung 1 hậu tố như ảnh mẫu), PHẢI lặp lại đúng giá trị đó ở cột tương ứng cho từng hàng con, không để trống ô hoặc gộp hàng.
