...
3. "grammarPoints": mỗi điểm ngữ pháp gồm tiêu đề và các đề mục giải thích ("sections"), mỗi đề mục có ví dụ minh hoạ đánh số RIÊNG của chính nó. TUYỆT ĐỐI KHÔNG trích xuất phần luyện tập/bài tập hỏi-đáp (練習/Luyện tập) của ngữ pháp.
   ...
   QUAN TRỌNG về bảng dữ liệu tham khảo (KHÁC với "bảng ví dụ nhiều cột" ở trên — đây là bảng liệt kê từ vựng/công thức/danh mục, KHÔNG chứa câu ví dụ đánh số (1)(2)(3)): ví dụ bảng liệt kê các từ chỉ vị trí kèm hậu tố tương ứng (上/下/前/後/裡/外... ghép với 面 hoặc 邊), bảng liệt kê địa danh theo nhóm, hoặc bảng công thức nhiều cột. TUYỆT ĐỐI KHÔNG BỎ QUA loại bảng này chỉ vì nó không phải câu ví dụ — PHẢI ghi lại TOÀN BỘ nội dung bảng vào "content" của đúng đề mục/ý chứa bảng đó (không phải "examples"), dưới dạng văn bản liệt kê rõ ràng theo từng dòng/từng nhóm, xuống dòng (\\n) giữa các mục, giữ đúng cặp chữ Hán - nghĩa/phiên âm đi kèm trong bảng gốc (ví dụ viết lại thành "上 trên, 下 dưới, 前 trước, 後 sau, 裡 trong, 外 ngoài + hậu tố 面 hoặc 邊" thay vì bỏ qua bảng). Nếu bảng nằm trong một đề mục/ý đã có đoạn giải thích văn xuôi khác, nối phần liệt kê từ bảng vào ngay sau đoạn giải thích đó trong cùng "content", cách nhau bằng xuống dòng.
